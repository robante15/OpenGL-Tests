...
float angle_1_cube = 0.0f; // Ángulo de rotación
...
void display()
{
    ...
    glPushMatrix();
    glScalef(2.0f, 0.5f, 1.0f); // Escalar el cubo
    glRotatef(angle_1_cube, 0.0f, 1.0f, 0.0f); // Rotar alrededor del eje y
    drawCube();
    glPopMatrix();
...
void idle()
{
    angle_1_cube += 0.5f; // Incrementar ángulo para animación
    if (angle_1_cube >= 360.0f)
        angle_1_cube -= 360.0f;
    glutPostRedisplay();
}
