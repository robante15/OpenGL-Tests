...
int sentido_x = -1; // Sentido de movimiento en X
int sentido_y = 1; // Sentido de movimiento en Y
...
void display(){
    ...
    glutSwapBuffers(); // Intercambia los buffers para mostrar el nuevo contenido
}
...
void idle(){
    triangle_pos_x += 0.01f * sentido_x; // Actualiza la posición en X
    triangle_pos_y += 0.01f * sentido_y; // Actualiza la posición en Y

    if(out_of_x_bounds(triangle_pos_x)){
        sentido_x *= -1; // Cambia el sentido si sale de los límites
    }

    if(out_of_y_bounds(triangle_pos_y)){
        sentido_y *= -1; // Cambia el sentido si sale de los límites
    }
    
    glutPostRedisplay(); // Solicita la redistribución de la ventana
}
...
int main(int argc, char** argv){
    // Inicializa GLUT
    glutInit(&argc, argv); 
    glutInitDisplayMode(GLUT_DOUBLE); // Establece el modo de visualización
    ...
    glutIdleFunc(idle); // Registra la función de inactividad
